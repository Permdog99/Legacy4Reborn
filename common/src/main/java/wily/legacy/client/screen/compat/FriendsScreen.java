...
public class FriendsScreen extends PanelVListScreen {
    ...
    public FriendsScreen(Screen parent, Component component) {
        super(s-> Panel.centered(s,260,214,0,30), component);
        this.parent = parent;
    ...
    public FriendsScreen(Screen parent) {
        this(parent, WORLD_HOST_FRIENDS);
    }

    @Override
    public void addControlTooltips(ControlTooltip.Renderer renderer) {
        super.addControlTooltips(renderer);
        renderer.add(()-> ControlType.getActiveType().isKbm() ? ControlTooltip.getKeyIcon(InputConstants.KEY_O) : ControllerBinding.UP_BUTTON.bindingState.getIcon(), ()-> io.github.gaming32.worldhost.gui.screen.FriendsScreen.ADD_FRIEND_TEXT);
    ...
    protected void init() {
        panel.init();
        renderableVList.init(this,panel.x + 19,panel.y + 31,(panel.width/2) - 18 - 11, 30 * 6);
    }
    ...
    public boolean keyPressed(int i, int j, int k) {
        if (i == InputConstants.KEY_O){
            ...
            minecraft.setScreen(new ConfirmationScreen(this,230,140, io.github.gaming32.worldhost.gui.screen.FriendsScreen.ADD_FRIEND_TEXT,WORLD_HOST_FRIEND_USERNAME_TEXT, b->{}){
                long lastTyping = -1;
                GameProfile friend;
                @Override
                protected void init() {
                    okAction = b-> {
                        if (friend != null) {
                            io.github.gaming32.worldhost.gui.screen.FriendsScreen.addFriend(friend);
                ...
                renderableVList.addRenderable(new AbstractButton(0, 0, 230, 30, Component.literal(WorldHost.getName(profile))) {
                    ...
                    public boolean keyPressed(int i, int j, int k) {
                        if (i == InputConstants.KEY_X) {
                            minecraft.setScreen(new ConfirmationScreen(FriendsScreen.this, WORLD_HOST_REMOVE_FRIEND, Components.translatable("world-host.friends.remove.title"), b -> {
    ...
    public void renderDefaultBackground(GuiGraphics guiGraphics, int i, int j, float f) {
        ...
        guiGraphics.blitSprite(LegacySprites.PANEL_RECESS, panel.x + panel.width/2, panel.y + 11, panel.width/2 - 10, panel.height - 23);
        guiGraphics.drawString(this.font, "Friends", (panel.x + panel.width/2) - (font.width("Friends") - font.width("Friends")/2),panel.y + 19, CommonColor.INVENTORY_GRAY_TEXT.get(), false);
    }
