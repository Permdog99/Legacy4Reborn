...
public class SaveRenderableListTU25 extends RenderableVList {
    ...
    protected PlayGameScreenTU25 screen;
    ...
    public SaveRenderableListTU25(PlayGameScreenTU25 PlayGameScreenTU25) {
        screen = PlayGameScreenTU25;
        layoutSpacing(l->0);
    ...
    public static void resetIconCache(){
        SaveRenderableListTU25.iconCache.asMap().forEach((s, i)-> i.close());
        SaveRenderableListTU25.iconCache.invalidateAll();
    }
                ...
                addRenderable(new AbstractButton(0, 0, 270, 30, Component.literal(summary.getLevelName())) {
                    ...
                    public boolean keyPressed(int i, int j, int k) {
                        if (i == InputConstants.KEY_O) {
                            minecraft.setScreen(new SaveOptionsScreenTU25(screen, summary));
    ...
    public PlayGameScreenTU25 getScreen() {
        return this.screen;
    }
    ...
    public void joinWorld(LevelSummary summary) {
        if (summary.primaryActionActive()) {
            if (summary instanceof LevelSummary.SymlinkLevelSummary) {
                ...
                SaveRenderableListTU25.this.reloadSaveList();
                if (((LegacyOptions)minecraft.options).directSaveLoad().get()){
                    Legacy4JClient.copySaveBtwSources(LoadSaveScreenTU25.getSummaryAccess(Minecraft.getInstance().getLevelSource(),summary),Legacy4JClient.currentWorldSource);
                    LoadSaveScreenTU25.loadWorld(screen,minecraft, Legacy4JClient.currentWorldSource,summary.getLevelId());
                }else minecraft.setScreen(new LoadSaveScreenTU25(screen, summary, Legacy4JClient.currentWorldSource){
                    @Override
